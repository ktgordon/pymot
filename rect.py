...
class Rect:
    ...
    def __str__(self):
        ...
        if self.id_ != "":
            ...
            return "(id, x,y,w,h) = (%s, %.1f, %.1f, %.1f, %.1f, %s)" % (self.id_, self.x_, self.y_, self.w_, self.h_, "DCO" if self.dco_ else "non-DCO")
        else:
            return "(x,y,w,h) = (%.1f, %.1f, %.1f, %.1f, %s)" % (self.x_, self.y_, self.w_, self.h_, "DCO" if self.dco_ else "non-DCO")
